...
public class Demo {
	...
	public static void main(String[] args) throws Exception {
		...
		String url="hdfs://192.168.0.102:9000";
		FileSystem fileSystem=FileSystem.get(URI.create(url),configuration);
//		Path path = new Path("/usr");service network restart
//		fileSystem.mkdirs(path);
		fileSystem.copyFromLocalFile(new Path("c:/hadoop.txt"),new Path( "/usr/hadoop.txt"));
		System.out.println("new dir \t" + configuration.get("fs.default.name") + "/usr");
//		fileSystem.delete(new Path("/usr"), true);
//		Path path = new Path("hdfs://192.168.0.100:9000/solr");  
